...
- destination photo at sunset generated once with **Gemini** and cached in `cache/photos/`
...
3. `scripts/generate-photos.js` asks Gemini for a text-free 9:16 photo of every destination that does
   not have one yet (max `MAX_NEW_PHOTOS` per run, default 100, so the cache grows incrementally).
...
4. `scripts/render.js` composites the copy on every cached photo → `dist/<market>/images/<key>.jpg`.
   No Gemini call here: price changes only re-render the overlay.
...
1. Repository secret `GEMINI_API_KEY` (Settings → Secrets and variables → Actions).
...
- `scripts/generate-photos.js`: `PROMPT_VERSION` (bump to regenerate every photo), model via `GEMINI_MODEL`.
...
node scripts/dev-placeholder.js                  # text-free placeholder photo, no Gemini needed
...
# real photos: GEMINI_API_KEY=... ONLY=rome-italy,paris-france node scripts/generate-photos.js us-en
```
